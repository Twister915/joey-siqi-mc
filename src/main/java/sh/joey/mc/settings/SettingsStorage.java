...
public final class SettingsStorage {
    ...
    public Maybe<PlayerSettings> getSettings(UUID playerId) {
        return storage.queryMaybe(conn -> {
            try (var stmt = conn.prepareStatement(
                    "SELECT keep_inventory, display_time, easy_mode, passive_mode FROM player_settings WHERE player_id = ?")) {
    ...
    public Completable saveSettings(UUID playerId, PlayerSettings settings) {
        return storage.execute(conn -> {
            try (var stmt = conn.prepareStatement("""
                    INSERT INTO player_settings (player_id, keep_inventory, display_time, easy_mode, passive_mode, updated_at)
                    VALUES (?, ?, ?, ?, ?, NOW())
                ...
                stmt.setBoolean(5, settings.passiveMode());
                stmt.executeUpdate();
            }
        });
    }
    ...
    public Flowable<Map.Entry<UUID, PlayerSettings>> getAllSettings() {
        return storage.queryFlowable(conn -> {
            ...
            try (var stmt = conn.prepareStatement(
                    "SELECT player_id, keep_inventory, display_time, easy_mode, passive_mode FROM player_settings")) {
                var rs = stmt.executeQuery();
                while (rs.next()) {
